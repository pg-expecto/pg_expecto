...
### `mchain_train_step()`
**Назначение:** основной шаг онлайн-обучения. Вызывается **каждую минуту** (например, из `performance_metrics`).  
...
**Назначение:** анализирует переходы в аварийные состояния (`correlation < 0 AND os_trend = -1`) за указанный интервал (по умолчанию последние 7 дней). Объединяет сырую статистику из `transition_log` и взвешенные частоты из `markov_frequencies` (текущая модель с учётом забывания). Возвращает текстовый отчёт со сравнением, динамикой и рекомендациями.  
**Возвращает:** `TEXT`.
...
### Сводный отчёт за последние 14 дней
```sql
SELECT mchain_summary_report(now() - interval '14 days', now());
```

### Детальный отчёт по аварийным состояниям
```sql
SELECT mchain_incident_state_detail_report();
```

### Проверка здоровья цепи Маркова для мониторинга
```sql
...
В файле `crontab.txt` приведены рекомендуемые задания для регулярного обслуживания:
...
Убедитесь, что путь к `psql` и параметры подключения настроены правильно.
