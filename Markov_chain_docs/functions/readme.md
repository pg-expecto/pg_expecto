...
Данный набор PL/pgSQL функций реализует **онлайн‑обучение цепи Маркова** на потоке метрик производительности (`корреляция`, `тренд операционной скорости`, `тренд ожидания`). Модель автоматически обновляется каждую минуту, адаптивно забывает устаревшие данные и выдаёт краткосрочные прогнозы риска аварий (инцидентов). Основная функция `mchain_train_step()` вызывается **каждую минуту** из процедуры сбора метрик `performance_metrics`. Средняя частота реальных инцидентов — **≈1 раз в день**, что используется для динамической настройки забывания (период полураспада веса инцидента — 7 дней).
...
- [Отчетные функции и мониторинг](#отчетные-функции-и-мониторинг)
- [Функции для работы с аварийными состояниями](#функции-для-работы-с-аварийными-состояниями)
- [Матрица переходов между макрогруппами](#матрица-переходов-между-макрогруппами)
...
### `mchain_train_step()`
**Назначение:** основной шаг онлайн-обучения. Вызывается **каждую минуту** из `performance_metrics`.  
...
- Для аварийных состояний (`correlation < 0 AND os_trend = -1 AND wait_trend = 1`) создаётся только петля с вероятностью 1.0.  
**Возвращает:** `VOID`.
...
> **Контекст:** частота инцидентов ~1 в день. Параметры адаптивного забывания (`base_alpha = 0.1`, `incident_half_life_days = 7.0`, `min_alpha = 0.01`) обеспечивают быструю реакцию на недавние аварии и плавное «забывание» при их отсутствии.
...
## Отчетные функции и мониторинг

### `mchain_incident_transitions_report(p_start TIMESTAMPTZ DEFAULT NULL, p_end TIMESTAMPTZ DEFAULT NULL)`
**Назначение:** анализирует переходы в аварийные состояния (`correlation < 0 AND os_trend = -1 AND wait_trend = 1`) за указанный интервал (по умолчанию последние 7 дней). Объединяет сырую статистику из `transition_log` и взвешенные частоты из `markov_frequencies` (текущая модель с учётом забывания). Возвращает текстовый отчёт со сравнением, динамикой и рекомендациями.  
**Возвращает:** `TEXT`.

### `mchain_summary_report(p_start TIMESTAMPTZ DEFAULT NULL, p_end TIMESTAMPTZ DEFAULT NULL)`
**Назначение:** сводный отчёт по состоянию цепи Маркова. Включает:
- общую достоверность прогнозов (`mchain_reliability_report`),
- анализ переходов в аварию за период (`mchain_incident_transitions_report`),
- текущие параметры конфигурации (забывание, пороги),
- текущее состояние системы (если доступно),
- прогноз риска на следующую минуту.  
**Возвращает:** `TEXT`.

### `mchain_incident_state_detail_report(p_start TIMESTAMPTZ DEFAULT NULL, p_end TIMESTAMPTZ DEFAULT NULL)`
**Назначение:** детализированный отчёт по каждому аварийному состоянию, в которое были переходы за период. Для каждого такого состояния показывает:
- сырую частоту за период,
- взвешенную частоту из `markov_frequencies`,
- топ‑3 предшествующих состояния с их вероятностями/частотами,
- дополнительную статистику по инцидентам производительности (таблица `performance_incident`).  
**Возвращает:** `TEXT`.

### `mchain_health_check()`
**Назначение:** проверяет работоспособность цепи Маркова и возвращает статус (`OK`, `WARNING`, `CRITICAL`) с подробным сообщением. Предназначена для систем мониторинга. Проверяет:
- достоверность прогнозов (рейтинг),
- рост аварийных переходов (последние 7 дней vs предыдущие 7 дней),
- давность последнего забывания,
- наличие свежих переходов (активность),
- заполненность `markov_frequencies`.  
**Возвращает:** `TABLE (status TEXT, message TEXT)`.
...
- стабильности вероятностей за 2 недели,
- покрытия частых состояний (частота >1% и ≥50 переходов).  
**Возвращает:** `INT`.
...
## Функции для работы с аварийными состояниями

### `decode_state_id(p_state_id SMALLINT)`
**Назначение:** обратная функция к `get_state_id`. По числовому идентификатору состояния возвращает его параметры: корреляцию (`r`), тренд операционной скорости (`os_trend`), тренд времени ожидания (`wait_trend`).  
**Возвращает:** `TABLE (r REAL, os_trend SMALLINT, wait_trend SMALLINT)`.

### `get_critical_states(only_with_transitions BOOLEAN DEFAULT TRUE)`
**Назначение:** возвращает список аварийных состояний, соответствующих критериям: `correlation < 0`, `os_trend = -1`, `wait_trend = 1`.  
- Если `only_with_transitions = TRUE` (по умолчанию) — только те состояния, которые реально встречались в `transition_log` как целевые.  
- Если `FALSE` — все подходящие состояния из справочника.  
**Возвращает:** `TABLE (state_id SMALLINT, correlation REAL, os_trend SMALLINT, wait_trend SMALLINT, last_seen TIMESTAMPTZ)`.

### `get_critical_state_ids(only_with_transitions BOOLEAN DEFAULT TRUE)`
**Назначение:** упрощённая версия `get_critical_states`, возвращает только идентификаторы аварийных состояний.  
**Возвращает:** `SETOF SMALLINT`.

---

## Матрица переходов между макрогруппами

### `mchain_state_transition_matrix_report(p_use_weighted BOOLEAN DEFAULT FALSE, p_include_wait_trend BOOLEAN DEFAULT FALSE)`
**Назначение:** формирует текстовую матрицу переходов между укрупнёнными группами состояний, агрегируя вероятности из `markov_probabilities`.  
- Группировка:
  - `p_include_wait_trend = FALSE`: 9 групп (знак корреляции × тренд OS).
  - `p_include_wait_trend = TRUE`: 27 групп (знак корреляции × тренд OS × тренд ожиданий).
- Способ усреднения:
  - `p_use_weighted = FALSE` — простое среднее вероятностей по состояниям в группе.
  - `p_use_weighted = TRUE` — взвешенное по частоте (`markov_frequencies.frequency`) среднее.
- Дополнительно выводится распределение переходов из **текущего состояния** (если определено).  
**Возвращает:** `TEXT` — отформатированная матрица.
...
### Минутное обучение (из performance_metrics)
```sql
