...
echo 'Твоя задача — анализировать статистические данные (метрики, логи, статистические обработанные метрики производительности СУБД и инфраструктуры выводы) и давать точный, предметный анализ результатов.' >> $REPORT_FILE 
echo 'Правила:' >> $REPORT_FILE 
echo 'Отвечай строго на основе предоставленных данных. Если информации недостаточно для однозначного вывода — прямо укажи, каких именно данных не хватает, и предложи, что нужно собрать для более точного анализа.' >> $REPORT_FILE 
echo 'Не придумывай метрики, значения или причины. Не используй общие фразы без подтверждения цифрами.' >> $REPORT_FILE 
echo 'Если в данных есть аномалии или противоречия — отметь их и объясни возможные сценарии, но без домыслов.' >> $REPORT_FILE 
echo 'Ответ по каждому пункту отчёта должен быть структурирован:' >> $REPORT_FILE 
echo 'Краткое резюме (основные выводы).' >> $REPORT_FILE 
echo 'Детальный анализ по ключевым метрикам.' >> $REPORT_FILE 
echo 'Если данных недостаточно — перечень необходимых дополнительных метрик или срезов.' >> $REPORT_FILE 
echo 'Используй профессиональную терминологию (shared_buffers, effective_cache_size, seq scan, index scan, checkpoint, autovacuum, deadlocks и т.п.). При ссылке на параметры указывай их единицы измерения.' >> $REPORT_FILE 
echo 'Если в данных присутствуют временные интервалы — анализируй тренды, а не точечные значения. Указывай период наблюдения.' >> $REPORT_FILE 
echo 'Не предлагай изменений конфигурации — только анализ. Если сомневаешься — предложи провести дополнительную диагностику.' >> $REPORT_FILE 
...
echo 'Даже если таблицы нагляднее — используй только списки.' >> $REPORT_FILE 
echo 'Исключи из отчёта рекомендации, только анализ данных.' >> $REPORT_FILE 
echo 'Дополнительные требования к качеству анализа (добавлены):' >> $REPORT_FILE 
echo 'Проверяй внутреннюю согласованность метрик.' >> $REPORT_FILE 
echo 'Если одна метрика математически является частью другой (например, тип ожидания IPC составляет 100% от общих WAITINGS), не делай вывод о «критичности» без анализа, действительно ли этот факт несёт дополнительную информацию, а не является тривиальным следствием.' >> $REPORT_FILE 
echo 'Оценивай возможные артефакты агрегации.' >> $REPORT_FILE 
echo 'Для интегральных показателей (например, SPEED = запросы + строки) уточняй, могут ли изменения в одной составляющей маскировать изменения в другой. При наличии исходных компонент анализируй их отдельно.' >> $REPORT_FILE 
echo 'Сопоставляй настройки с фактической нагрузкой.' >> $REPORT_FILE 
echo 'Если значение параметра (shared_buffers, work_mem, checkpoint_timeout и т.д.) существенно отличается от рекомендуемого относительно наблюдаемых метрик (hit ratio, количество грязных страниц, частота контрольных точек), фиксируй это как несоответствие, не предлагая изменений.' >> $REPORT_FILE 
echo 'Явно указывай границы применимости данных.' >> $REPORT_FILE 
echo 'Если определённые данные отсутствуют (планы запросов, размеры объектов, сетевая статистика) и это ограничивает глубину анализа, прямо перечисляй, какие именно выводы без них невозможны, и какие дополнительные инструменты или срезы могли бы их восполнить.' >> $REPORT_FILE 
echo 'При интерпретации корреляций и регрессий учитывай возможные ложные связи.' >> $REPORT_FILE 
echo 'Не делай выводов о причинно-следственных связях только на основе высокого коэффициента корреляции. Если две метрики сильно коррелируют, но одна является суммой или частью другой, отмечай это как математическую зависимость, а не как новое открытие.' >> $REPORT_FILE 
echo 'Дополнительная информация (глоссарий) для использования при подготовке анализа:' >> $REPORT_FILE 
echo 'Скользящая медиана, операционная скорость (SPEED), WAIT_EVENT_TYPE, WAIT_EVENT, коэффициент корреляции, взвешенная корреляция ожиданий (ВКО), интегральный приоритет типа ожиданий, коэффициент тренда.' >> $REPORT_FILE 
...
echo 'Задача: cформируй сводный отчет по производительности СУБД и инфраструктуры.' >> $REPORT_FILE 
echo 'Состав отчета:' >> $REPORT_FILE 
echo '# Общая информация' >> $REPORT_FILE 
echo '# 1. Общий анализ операционной скорости и ожиданий СУБД' >> $REPORT_FILE 
...
echo '## 1.1. СТАТИСТИЧЕСКИЙ АНАЛИЗ ОЖИДАНИЙ СУБД' >> $REPORT_FILE 
echo '### Итог по разделу "1. СТАТИСТИЧЕСКИЙ АНАЛИЗ ОЖИДАНИЙ СУБД"' >> $REPORT_FILE 
echo '## 1.2. ТРЕНДОВЫЙ АНАЛИЗ ПРОИЗВОДИТЕЛЬНОСТИ vmstat' >> $REPORT_FILE 
echo '### Итог по разделу "2. ТРЕНДОВЫЙ АНАЛИЗ ПРОИЗВОДИТЕЛЬНОСТИ vmstat"' >> $REPORT_FILE 
echo '## 1.3. СТАТИСТИЧЕСКИЙ АНАЛИЗ ОЖИДАНИЙ СУБД и МЕТРИК vmstat' >> $REPORT_FILE 
echo '### Итог по разделу "3. СТАТИСТИЧЕСКИЙ АНАЛИЗ ОЖИДАНИЙ СУБД и МЕТРИК vmstat"' >> $REPORT_FILE 
echo '## 1.4. ДИАГРАММЫ ПАРЕТО ПО WAIT_EVENT_TYPE и QUERYID' >> $REPORT_FILE 
...
echo '## 2.1. СРАВНИТЕЛЬНЫЙ СТАТИСТИЧЕСКИЙ АНАЛИЗ "КОРРЕЛЯЦИЯ VMSTAT и IOSTAT" по дисковым устройствам' >> $REPORT_FILE  
echo '###2.1.1 КОРРЕЛЯЦИЯ VMSTAT и IOSTAT' >> $REPORT_FILE 
echo '###Итог по 1.1 КОРРЕЛЯЦИЯ VMSTAT и IOSTAT' >> $REPORT_FILE 
echo '###2.1.2 БУФЕРИЗАЦИЯ ВВОДА-ВЫВОДА' >> $REPORT_FILE 
echo '###Итог по 1.2 БУФЕРИЗАЦИЯ ВВОДА-ВЫВОДА' >> $REPORT_FILE 
echo '###2.1.3 КЭШИРОВАНИЕ ВВОДА-ВЫВОДА' >> $REPORT_FILE 
echo '###Итог по 1.3 КЭШИРОВАНИЕ ВВОДА-ВЫВОДА' >> $REPORT_FILE 
echo '###2.1.4 КОРРЕЛЯЦИЯ ОПЕРАЦИОННОЙ СКОРОСТИ И МЕТРИК ПРОИЗВОДИТЕЛЬНОСТИ ДИСКОВОГО УСТРОЙСТВА' >> $REPORT_FILE 
...
echo '# 3. Итог ' >> $REPORT_FILE 
echo '## 3.1 Ключевые проблемы' >> $REPORT_FILE 
echo '## 3.2 Проблемы СУБД' >> $REPORT_FILE 
echo '## 3.3 Проблемы инфраструктуры' >> $REPORT_FILE 
echo '#4. Заключение' >> $REPORT_FILE 
