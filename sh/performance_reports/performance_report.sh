...
if [ $# -ne 2 ]
then
  echo './performance_report.sh start_time finish_time'
  echo 'TIMESTAMP : '$(date "+%d-%m-%Y %H:%M:%S") ' : ERROR :  summary_report :  2 PARAMETERS MUST BE SET '
  echo 'TIMESTAMP : '$(date "+%d-%m-%Y %H:%M:%S") ' : ERROR :  summary_report :  2 PARAMETERS MUST BE SET ' >> $LOG_FILE  
  exit 0
fi 
...
# 3.СТАТИСТИКА VMSTAT - IOSTAT
# ЛОГ ФАЙЛ - ERRORS , AUTOVACUUM , CHECKPOINT
...
zip performance_4deepseek.zip  _1.settings.txt _2.postgresql_vmstat_iostat.txt pg_expecto_instruction.txt prompt_source.txt prompt_result.txt
exit_code $? $LOG_FILE $ERR_FILE
